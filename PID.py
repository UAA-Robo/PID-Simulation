...
class PID:
    ...
    def update_constants(self, setpoint:float = None, P: float = None, I: float = None,  D: float = None) -> None:
        ...
        :param P:           Proportional constant (how large to change to reach setpoint)
    ...
    def update_control_value(self, process_value, change_in_time) -> float:
        """ Approximation of PID to change the control value based on the new value
        :param process_value:       Value is affected by the control value (what want to reach the setpoint)
