...
class ParameterSpinbox(ctk.CTkFrame):
    ...
    def __init__(self, master:any, controller_parameters: list[ControllerParameter]):
        """
        ...
        @param controller_parameters    Controller Parameter to display in  a list. If there is more 
                                        then one, all will be set to whatever is displayed.
        @param update_period    Number of seconds in between updating the Parameter Value in this
                                frame.
        """
        if controller_parameters == []: return 
        ...
        self.label = ctk.CTkLabel(self,text = controller_parameters[0].name)
        ...
        self.entry.insert(0, self.controller_parameters[0].value) # Default value
    ...
    def update_entry_callback(self, event):
        ...
        for controller_parameter in self.controller_parameters:
            controller_parameter.value = self.entry.get()
    ...
    def add_button_callback(self):
        ...
        for controller_parameter in self.controller_parameters:
            try:
                controller_parameter.value += controller_parameter.adjust_amount
                self.entry.delete(0, "end")
                self.entry.insert(0, f"{controller_parameter.value:.2f}")
            except ValueError:
                return
